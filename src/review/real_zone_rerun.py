...
for name in ("testgrid_A", "testgrid_B", "doubleline"):
    ...
    sel = np.where(R["pos"] | R["neg"] | R["own99"] | R["parallel"] | R["reverse_bus"] | R["reverse_lines"] | R["switching"])[0]
    res = {}
    for rung, mim in (("R0", False), ("R2", True)):
        p = ld.pack(R, sel, 40, "full", mim)
        trip, _ = ld.evaluate_block(R, *p, S, rung)
        r = ld.rates(trip, R, sel)
        by = {}
        for cls in ("pos", "neg", "own99", "parallel", "reverse_bus"):
            m = R[cls][sel]
            if m.any():
        ...
        print(f"[{name}] {rung}: dep {r['pos']['rate']*100:5.1f}  out-of-zone {r['neg']['rate']*100:5.1f}  own99 "
              f"{r['own99']['rate']*100:5.1f}  reverse bus {r['reverse_bus']['rate']*100:5.1f} ({r['reverse_bus']['k']}/{r['reverse_bus']['n']})  "
              f"switching {r['switching']['k']}/{r['switching']['n']}"

...
def enable():
    """Route every load through the relay-bandwidth front end (chain key relay_aa=[ORDER, FC])."""
    ...
    def load_relay(name, chain=None, cubicle=None):
        ch = dict(chain or {}); ch.setdefault("relay_aa", [ORDER, FC])
        return orig_load(name, ch, cubicle)
    load_relay.__wrapped__ = orig_load
    cm.load_relay = load_relay
    return dict(relay_aa=[ORDER, FC])
...
if __name__ == "__main__":
    ...
    if what == "zone_cv":
        ...
        ap.add_argument("what"); ap.add_argument("relay")
        ap.add_argument("--times", type=int, nargs="+", default=[10, 20])
        ap.add_argument("--splits", nargs="+", default=["grouped"])
        a = ap.parse_args()
        res = zone_cv.run(a.relay, a.times, a.splits, "guard", chain=dict(relay_aa=[ORDER, FC]))
        json.dump(res, open(os.path.join(cm.ROOT, "logs", f"zone_cv_{a.relay}_relayfe.manifest.json"), "w"), indent=1)
    elif what == "ladder":
        from review import ladder
        out = {}
        for name in ("testgrid_B", "testgrid_A", "doubleline"):
            out[name] = ladder.run(name)
            json.dump(out, open(os.path.join(cm.ROOT, "results", "review", "ladder_relay_frontend.json"), "w"), indent=1, default=str)
